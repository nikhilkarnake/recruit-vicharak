...
typedef enum {
    ...
    TOKEN_PLUS, TOKEN_MINUS, TOKEN_IF, TOKEN_PRINT, TOKEN_LBRACE, TOKEN_RBRACE,
    TOKEN_COMMA, TOKEN_SEMICOLON, TOKEN_LPAREN, TOKEN_RPAREN, TOKEN_EOF
} TokenType;
...
void getNextToken(FILE *file, Token *token) {
    ...
    while ((c = fgetc(file)) != EOF) {
        ...
        if (isalpha(c)) {
            ...
            if (strcmp(token->text, "var") == 0) token->type = TOKEN_INT;
        ...
        switch (c) {
            ...
            case ',': token->type = TOKEN_COMMA; token->text[0] = ','; token->text[1] = '\0'; return;
            case ';': token->type = TOKEN_SEMICOLON; token->text[0] = ';'; token->text[1] = '\0'; return;
            default: token->type = TOKEN_EOF; token->text[0] = '\0'; return;
        }
    }
